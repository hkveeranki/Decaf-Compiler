...
static Module *TheModule = new Module("Decaf compiler jit", Context);
...
static AllocaInst *CreateEntryBlockAlloca(Function *TheFunction, const std::string &VarName, string type) {
    ...
    AllocaInst *Alloca;
    if (type == "int") {
        Alloca = TmpB.CreateAlloca(Type::getInt32Ty(Context), 0, VarName.c_str());
    } else if (type == "boolean") {
        Alloca = TmpB.CreateAlloca(Type::getInt1Ty(Context), 0, VarName.c_str());
    }
    return Alloca;
}
...
Value *fieldDecl::codegen() {
    for (int i = 0; i < var_list.size(); i++) {
        ...
        if (var->isArray()) {
            ...
            GlobalVariable *gv = new GlobalVariable(*TheModule, arrType, false, GlobalValue::ExternalLinkage, 0,
                                                    var->getName());
            gv->setInitializer(ConstantAggregateZero::get(arrType));
        } else {
            PointerType *ptrTy = PointerType::get(ty, 0);
            GlobalVariable *gv = new GlobalVariable(*TheModule, ptrTy, false, GlobalValue::ExternalLinkage, 0,
                                                    var->getName());
        }
    }
